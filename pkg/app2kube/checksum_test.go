...
// dataChecksum must be deterministic (order-independent), use the documented
// canonical "key=value\n" form so the value is stable and predictable, and
// change whenever the data changes — it backs the checksum/* pod annotations
// that roll a workload on config/secret change (#22).
func TestDataChecksum(t *testing.T) {
	...
	// Canonical form: sorted "key=value\n" lines, sha256, lowercase hex.
	want := fmt.Sprintf("%x", sha256.Sum256([]byte("A=1\nB=2\n")))
	if a != want {
		t.Errorf("checksum format changed: got %s, want %s", a, want)
	}
